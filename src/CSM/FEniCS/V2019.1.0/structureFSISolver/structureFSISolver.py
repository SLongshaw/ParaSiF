...
class StructureFSISolver:
    ...
    def __init__(   self, 
        ...
        # F-The RBF matrix will not write to file (if read matrix disabled); T-The RBF matrix will write to file.
        self.iWriteMatrix = self.configure['MUI'].getboolean('iWriteMatrix')
        ...
        # RBF Eigen Conjugate Gradient solver tolerance (float)
        self.cgSolveTol = float(self.configure['MUI']['cgSolveTol'])
        # RBF Eigen Conjugate Gradient solver maximim iterations (integer)
        self.cgMaxIter = int(self.configure['MUI']['cgMaxIter'])
        # RBF Partition of Unity patch size (integer)
        self.pouSize = int(self.configure['MUI']['pouSize'])
    ...
    def Solver(self):
        ...
        if self.iMUICoupling:
            if self.iUseRBF:
                if self.iMUIFetchForce:
                    if self.iparallelFSICoupling:
                        t_sampler, s_sampler = self.MUI_Sampler_RBF_Define(MPI_COMM_WORLD, ifaces3d, dofs_fetch_list, dofs_to_xyz, dofs_push_list, dofs_to_xyz, xyz_fetch_list_total_group, t_step)
                    else:
                        t_sampler, s_sampler = self.MUI_Sampler_RBF_Define(MPI_COMM_WORLD, ifaces3d, dofs_fetch_list, dofs_to_xyz, dofs_push_list, dofs_to_xyz, xyz_fetch_list_total_group, t_step)
                else:
                    if self.iparallelFSICoupling:
                ...
                if self.iMUIFetchForce:
                    t_sampler, s_sampler = self.MUI_Sampler_Define(MPI_COMM_WORLD, ifaces3d, dofs_fetch_list, xyz_fetch, dofs_push_list, xyz_push, t_step)
                else:
                    t_sampler, s_sampler = self.MUI_Sampler_Define(MPI_COMM_WORLD, ifaces3d, dofs_fetch_list, xyz_fetch, dofs_push_list, xyz_push, t_step)
        else:
            pass
